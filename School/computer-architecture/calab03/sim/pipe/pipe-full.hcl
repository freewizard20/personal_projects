...
word f_pc = [
	...
	E_icode == IJXX && !M_Cnd && !(M_icode in {IOPQ,IIADDQ}) && E_ifun == M_ifun && D_icode == INOP : E_valA;
	D_icode == IJXX && !M_Cnd && !(E_icode in {IOPQ,IIADDQ}) && !(M_icode in {IOPQ, IIADDQ, IJXX, IRRMOVQ }) && D_ifun == M_ifun : D_valP;
...
word f_ifun = [
	imem_error : FNONE;
	!(imem_icode in {IOPQ,IJXX,IRRMOVQ}) : 1;
	1: imem_ifun;
];
...
word aluA = [
	E_icode == IOPQ && M_icode in {IMRMOVQ, IPOPQ} && M_dstM == E_srcA : m_valM;
...
word aluB = [
	E_icode == IOPQ && M_icode in {IMRMOVQ, IPOPQ} && M_dstM == E_srcB : m_valM;
...
word e_valA = [
	E_srcA == M_dstM && M_icode in {IMRMOVQ, IPOPQ} && E_icode in {IRMMOVQ, IPUSHQ}:m_valM;
...
bool F_stall =
	...
	((E_dstM == d_srcA && !(D_icode in {IRMMOVQ, IOPQ}) && !(D_icode == IPUSHQ && f_icode == IRET)) || (E_dstM == d_srcB && D_icode != IOPQ)) ||((D_icode == IRET && E_icode != IPUSHQ)||(E_icode == IRET && M_icode != IPUSHQ) || (M_icode == IRET && W_icode != IPUSHQ));
...
bool D_stall = 
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
	 ((E_dstM == d_srcA && !(D_icode in {IRMMOVQ, IOPQ}) && !(D_icode == IPUSHQ && f_icode != IRET)) || (E_dstM == d_srcB && D_icode != IOPQ));

bool D_bubble =
	(D_icode == IJXX && !e_Cnd && !(E_icode in {IOPQ,IIADDQ}) && D_ifun == E_ifun && !(!(M_icode in {IOPQ,IIADDQ}) && D_ifun == M_ifun)) || (E_icode == IJXX && !e_Cnd && !(!(M_icode in {IOPQ,IIADDQ}) && E_ifun == M_ifun) && !(!(M_icode in {IOPQ,IIADDQ}) && !(W_icode in {IOPQ,IIADDQ, IJXX, IRRMOVQ}) && E_ifun == 1)) || ((D_icode == IRET && E_icode != IPUSHQ) ||(E_icode == IRET && M_icode != IPUSHQ) || (M_icode == IRET && W_icode != IPUSHQ)) && !(E_icode in {IMRMOVQ, IPOPQ} && ((E_dstM == d_srcA && !(D_icode in {IRMMOVQ, IOPQ}) && !(D_icode == IPUSHQ && f_icode != IRET)) || (E_dstM == d_srcB && D_icode != IOPQ)));

# Should I stall or inject a bubble into Pipeline Register E?
# At most one of these can be true.
bool E_stall = 0;
bool E_bubble =	(E_icode == IJXX && !e_Cnd && !(!(M_icode in {IOPQ, IIADDQ}) && !(W_icode in {IOPQ,IIADDQ,IJXX,IRRMOVQ}) && E_ifun == 1)) || E_icode in { IMRMOVQ, IPOPQ } && ((E_dstM== d_srcA && !(D_icode in {IRMMOVQ, IPUSHQ, IOPQ})) || (E_dstM == d_srcB && D_icode != IOPQ));
