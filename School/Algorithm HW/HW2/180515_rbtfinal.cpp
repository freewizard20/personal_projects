...
public:
	...
	int rank(int x);
	// tester functions
	bool checknil(node * x){
		bool ret = 1;
		if(x==root){
			if(root->parent!=nil){
				cout << "rootnil vio" << endl;
				return 0;
			}
		}
		if(x==nil){
...
int rbt::selecth(node * t, int x) {
	if (t == nil) return 0;
	int curr = (t->left == nil) ? 0 : t->left->rank;
	if (curr == x) return t->value;
	else if (curr > x) return selecth(t->left, x);
	else return selecth(t->right, x - (curr + 1));
}
...
int rbt::rank(int x) {
	...
	int ret = (here->left == nil) ? 1 : here->left->rank + 1;
	if(here==root) return ret;
	while (here != root) {
		if (here == here->parent->right) {
			ret += (here->parent->left == nil) ? 1 : here->parent->left->rank + 1;
		}
		follower = nil;
		here = here->parent;
	}
	//ret += (root->left==follower)?0:(root->left==nil)?0:root->left->rank+1;
	return ret;
}
