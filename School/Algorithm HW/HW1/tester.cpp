...
int main(int argc, char ** argv) {
	...
	long long detres = 0;
	for(int i = 0 ; i < 30 ; i++){
	vector<int> tmpbook = book;
	auto start = chrono::high_resolution_clock::now();
	int sam = randomfindKthelement(tmpbook, K, 0, book.size() - 1);
	auto end = chrono::high_resolution_clock::now();
	if(randres<(end-start).count()) randres = (end-start).count();
	//cout << i << "th rep done found " << sam << " took " << (double)(end-start).count()/1000000 << "ms" << endl;
	}
	//printer(0, (end - start).count(), randres, K);
	for(int i = 0  ; i < 30 ; i++){
	vector<int> tmpbook = book;
	auto start2 = chrono::high_resolution_clock::now();
	int sam = detfindKthelement(tmpbook, K, 0, book.size() - 1);
	auto end2 = chrono::high_resolution_clock::now();
	//printer(1, (end2 - start2).count(), detres, K);
	if(detres<(end2-start2).count()) detres = (end2-start2).count();
	//cout << i << "th det rep done found " << sam << " took " << (double)(end2-start2).count()/1000000 << "ms" << endl;
	}
	cout << "Worst runtime random " << ((double)randres/(double)1000000) << "ms" << endl;
	cout << "Worst runtime det " << ((double) detres / (double) 1000000) << "ms" << endl;
}
