...
class avl<T>{
	...
	// debugging function to print tree structure
	public void printtree(){
		pt(root,0);
	}

	public void pt(node<T> curr, int d){
		if(curr==null) return;
		pt(curr.left,d+1);
		for(int i = 0 ; i < d*2 ; i++){
			System.out.print(" ");
	...
	public void insert(T obj,ArrayList<Integer> z) {
		...
		while(true) {
			...
			if(runner==null) {
				...
				node<T> writer = walker.parent;
				int bf=10;
				if(writer!=null) bf = height(writer.left)-height(writer.right);
				while(writer!=null && bf<2 && bf>-2){
					runner = walker;
					walker = writer;
				...
				if(writer!=null){
					if(writer.left==walker) {
						if(runner==walker.left) {
							writer = rightrotate(writer);
						}else if(runner==walker.right) {
							walker = leftrotate(walker);
							writer = rightrotate(walker.parent);
						}else{
						}
					}else {
						if(runner==walker.right) {
							writer = leftrotate(writer);
						}else if(runner==walker.left) {
							walker = rightrotate(walker);
							writer = leftrotate(walker.parent);
						}else{
						}
					}

					// rearrange height affected to the parents of writer
					node<T> arranger2 = writer;
					while(arranger2!=null){
						arranger2.height = Math.max(height(arranger2.left),height(arranger2.right))+1;
						arranger2 = arranger2.parent;
					}
