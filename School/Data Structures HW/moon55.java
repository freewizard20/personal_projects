...
public class Matching
{
	...
	public static void main(String args[])
	{
		...
		tmp.insert("100",new ArrayList<Integer>());
		tmp.insert("50",new ArrayList<Integer>());
		tmp.insert("150",new ArrayList<Integer>());
		tmp.printtree();
...
class avl<T>{
	...
	public void pt(node<T> curr, int d){
		...
		if(curr==null) return;
		System.out.println("in "+curr.data);
		pt(curr.left,d+1);
		System.out.println("passed " + curr.data);
		for(int i = 0 ; i < d*2 ; i++){
			System.out.print(" ");
		}
	...
	public int checkheight(node<T> curr){
		...
		if(curr.height!=height) System.out.println("fucked here");
		return height;
	}
				...
				if(writer!=null){
					...
					if(writer.left==walker) {
						...
						if(runner==walker.right) {
							...
							leftrotate(walker.parent);
						}else{
						System.out.println("pitfall2");
						}
					}
				}*/
